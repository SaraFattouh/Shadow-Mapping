...
CMyApp::CMyApp(int w_init, int h_init)
{
	...
	//camera settings (at, center, up)
	camera.SetView(glm::vec3(5, 5, 5), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));

	//Set the camera projection (fow, aspect ratio, near and far clipping distance)
...
void CMyApp::LoadAssets()
{
	...
	mesh_rocks1 = ObjParser::parse("rocks.obj");
	tex_rocks.FromFile("rock.jpg");
	std::cout << "rocks assets loaded\n";
...
inline void setTexture2DParameters(GLenum magfilter = GL_LINEAR, GLenum minfilter = GL_LINEAR, GLenum wrap_s = GL_CLAMP_TO_EDGE, GLenum wrap_t = GL_CLAMP_TO_EDGE)
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, magfilter); //use Mag fitering
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, minfilter);

	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, wrap_s); // wrapping //werap_S
...
void CMyApp::CreateFrameBuffers()
{
	...
	if (frameBufferCreated)
	{
		...
		//hold texture data, can't read or write to them directly, we need to allocate

		glDeleteRenderbuffers(1, &depthBuffer); //depth buffer 
		glDeleteFramebuffers(1, &fbo);
		glDeleteTextures(1, &shadow_depth_texture);
		glDeleteFramebuffers(1, &shadow_fbo);
	}

	//create framebuffer with textures attacheted to it
	glGenFramebuffers(1, &fbo); 
	glBindFramebuffer(GL_FRAMEBUFFER, fbo);

	// (Attachment 0.) Target for the base (texture) color of pixels
	glGenTextures(1, &colorBuffer);
	//allocate data for that texture, creates GL_TEXTURE2D with 32 bit per channel(4), width, height space for it on the GPU
	glBindTexture(GL_TEXTURE_2D, colorBuffer);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA, GL_FLOAT, nullptr); // last 3 parameters are only for initial values when the texture is loaded
	setTexture2DParameters(GL_NEAREST, GL_NEAREST); // texture will be read as that
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, colorBuffer, 0); //set texture to the framebuffer  (0) which bitmap we would like to use 
	//attaching colorbuffer as GL_TEXTURE_2D, we are attaching it as the very first attachement GL_COLOR_ATTACHMENT0
	...
	// Completeness check 
	//checking framebuffer requirements
...
void CMyApp::DrawScene(glm::mat4 waterLevel)
{
	...
	programForwardRenderer.SetUniform("specular_power", 20.0f);
...
void CMyApp::Render()
{
	...
	glDepthMask(GL_TRUE); //depth values are written by the shader

	glDisable(GL_BLEND); // fragment of the draw buffer will be overwritten

	// blending when you have a fragment and you want to write the 
	//glBlendEquation(GL_FUNC_ADD);				//color to the buffer and the buffer also has color
	//glBlendFunc(GL_ONE, GL_ONE);				//These 2 values can be blended  together with a function (Blending equation)


	// Clear it
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); //write to every fragment with a constant color
	// Run shader program
	...
	//depth texture
	programLightRenderer.SetTexture("shadowDepthTexture", 4, shadow_depth_texture);

	// send camera matrices
	programLightRenderer.SetUniform("projMatrixInv", glm::transpose(m_light_proj));
	programLightRenderer.SetUniform("viewMatrixInv", glm::transpose(m_light_view));

	glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);

	programLightRenderer.Unuse();

	if (ImGui::Begin("Base Color")) //if that window is open, if we close it the code will not run
	{
